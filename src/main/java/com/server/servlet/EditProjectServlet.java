...
public class EditProjectServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (formItems != null && formItems.size() > 0) {
                ...
                for (FileItem item : formItems) {
                    ...
                    {
                        if("p_id".equals(item.getFieldName()))
                            pf.setP_id(Integer.parseInt(new String(item.getString().getBytes(
                                    "ISO-8859-1"), "UTF-8")));
                        ...
                        if ("p_time".equals(item.getFieldName()))
                            pf.setP_time(new String(item.getString().getBytes(
                                    "ISO-8859-1"), "UTF-8"));
                        if ("p_word".equals(item.getFieldName())&&pf.getP_word().equals(""))
                            pf.setP_word(new String(item.getString().getBytes(
                                    "ISO-8859-1"), "UTF-8"));
                        if ("p_code".equals(item.getFieldName())&&pf.getP_code().equals(""))
                            pf.setP_code(new String(item.getString().getBytes(
                                    "ISO-8859-1"), "UTF-8"));
                        if ("p_video".equals(item.getFieldName())&&pf.getP_video().equals(""))
                            pf.setP_video(new String(item.getString().getBytes(
                                    "ISO-8859-1"), "UTF-8"));
                    }
                }
            }
        ...
        if(new ProjectFileDao().updataProjectFile(pf))
