...
@WebServlet(name = "AddCompetitionServlet",urlPatterns = "/Competition/AddCompetitionServlet")
public class AddCompetitionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
            ...
            if (formItems != null && formItems.size() > 0) {
                ...
                for (FileItem item : formItems) {
                    ...
                    if (!item.isFormField()) {
                        ...
                        }else if("video".equals(item.getFieldName())){
                            String fileName = new File(item.getName()).getName();
                            if(fileName.equals("")){
                                cp.setC_video("");
                                continue;
                        ...
                        }else if("certificate".equals(item.getFieldName())){
                            String fileName = new File(item.getName()).getName();
                            if(fileName.equals("")){
                                cp.setC_certificate("");
                                continue;
                    ...
                    else
                    {
                        if("u_id".equals(item.getFieldName()))
                            cp.setU_id(Integer.parseInt(new String(item.getString().getBytes(
                                    "ISO-8859-1"), "UTF-8")));
        ...
        if(new CompetitionDao().insertCompetition(cp))
            request.setAttribute("result","true");
        else
            request.setAttribute("result","false");

        request.getRequestDispatcher("/Competition/CompetitionAdd.jsp").forward(request, response);
    }
